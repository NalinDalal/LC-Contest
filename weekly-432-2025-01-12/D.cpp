...
For each subarray, track the required operations to make it non-decreasing.
If the total required operations exceed k, contract the window from the left to
reduce the operations. Count valid subarrays by checking the number of subarrays
that can be formed with the current window.*/
