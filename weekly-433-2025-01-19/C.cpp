/* https://leetcode.com/contest/weekly-contest-433/problems/paint-house-iv/?slug=maximum-and-minimum-sums-of-at-most-size-k-subsequences&region=global_v2
...
equidistant. Return the minimum cost to paint the houses such that they look
beautiful.
...
color (1 != 2). Houses at positions 1 and 2 (equidistant from the ends) are not
painted the same color (2 != 3). The minimum cost to paint the houses so that
they look beautiful is 3 + 2 + 1 + 3 = 9.
...
color (1 != 2). Houses at positions 1 and 4 (equidistant from the ends) are not
painted the same color (3 != 1). Houses at positions 2 and 3 (equidistant from
the ends) are not painted the same color (2 != 3). The minimum cost to paint the
houses so that they look beautiful is 2 + 8 + 1 + 2 + 3 + 2 = 18.
...
public:
  long long minCost(int n, vector<vector<int>> &cost) {
    // cost.size()=n
    // cost[i][j]=colouring house i with color j+1
    // no 2 adjacent are with same colour
