...
Output: 4
Explanation:
After adding 8 to nums[1..9], 10 has a frequency of 4 in [10, 10, 11, 12, 13,13,
12, 11, 10, 10].
...
public:
  int maxFrequency(vector<int> &nums, int k) {
    // select sub-array i,j
    // add x to all elements in sub-array
    // return totalcount of k in sub-array
    int n = nums.size();
    int totalCount = 0;
    for (int i = 0;)
      return totalCount;
