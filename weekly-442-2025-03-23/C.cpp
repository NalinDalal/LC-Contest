...
class Solution {
public:
  bool isPossible(long long T, vector<int> &skill, vector<int> &mana) {

    // n wizard produce m potions
    // each potion has capacity mana[j]
    // time by ith wizard on jth potion{timeij}=skill[i]*mana[j]
    // time should be synchronised
    // return min time required in range [1,max(skill)*max(mana)]-> apply binary
    // search on it bipartite matching problem, which can be solved greedily.
  }
};
